...
/**
 * Memastikan SDK Puter.js terunduh dari CDN secara on-the-fly dan dapat di-require secara dinamis.
 */
async function ensurePuterSdk() {
  const cacheDir = path.join(process.cwd(), '.yuihime', 'cache');
  const cachePath = path.join(cacheDir, 'puter_sdk.js');
  
  if (fs.existsSync(cachePath)) {
    try {
      return require(cachePath);
    } catch (err) {
      log(`Gagal memuat cached Puter SDK: ${err.message}. Mengunduh ulang...`);
    }
  }
  ...
  if (!fs.existsSync(cacheDir)) {
    try {
      ...
      log(`Gagal membuat folder cache: ${err.message}`);
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      https.get(url, (res) => {
        ...
        if (statusCode >= 300 && statusCode < 400 && res.headers.location) {
          ...
          return resolve(fetchWithRedirects(redirectUrl, depth + 1));
        }
        if (statusCode !== 200) {
  ...
  for (const url of urls) {
    try {
      log(`Mengunduh Puter SDK dari CDN: ${url}`);
      const code = await fetchWithRedirects(url);
      fs.writeFileSync(cachePath, code, 'utf8');
      log(`Puter SDK diunduh dan disimpan di ${cachePath}`);
      return require(cachePath);
    } catch (err) {
      lastErr = err;
      log(`Gagal mengunduh dari ${url}: ${err.message}`);
    }
  }
  
  throw new Error(`Semua CDN gagal dijangkau. Terakhir: ${lastErr ? lastErr.message : 'Unknown'}`);
}
...
class PuterBridge {
  ...
  async executeCall(action, params) {
    // Membuka dukungan native SDK prima dari Puter.js
    let puter = null;
    try {
      puter = await ensurePuterSdk();
      if (this.token) {
        puter.authToken = this.token;
      }
      log('[PUTER-SDK] SDK terdeteksi dan diinisialisasi secara native.');
    } catch (sdkLoadErr) {
      log('[PUTER-SDK] Gagal memuat atau menginisiasi Puter SDK native, menggunakan opsi REST fallback. Error: ' + sdkLoadErr.message);
    }
    ...
    try {
      if (action === 'chat') {
        ...
        // Coba lewat SDK native terlebih dahulu jika tersedia
        if (puter) {
          try {
            log(`[PUTER-SDK] Melakukan pemrosesan obrolan native via model: ${modelName} (${provider}).`);
            const responseTxt = await puter.ai.chat(params.input, {
              model: modelName,
              provider: provider
            });

            const textValue = typeof responseTxt === 'string' ? responseTxt : (responseTxt?.message || responseTxt?.text || responseTxt?.choices?.[0]?.message?.content || JSON.stringify(responseTxt));
            if (textValue) {
              return {
                text: textValue,
                model: modelName,
            ...
            log(`[PUTER-SDK] Obrolan native gagal: ${sdkChatErr.message}. Beralih ke REST fallback.`);
          }
        }
        ...
        // Coba endpoint chat yang mungkin (HTTP REST fallback)
        const endpoints = ['/ai/chat', '/puterai/chat'];
        let lastChatErr;

        for (const ep of endpoints) {
          try {
            log(`Mencoba chat REST via ${ep}...`);
      ...
      if (action === 'kv_set') {
        if (puter) {
          try {
            log('[PUTER-SDK] Menyimpan KV native...');
            await puter.kv.set(params.key, params.value);
            return { success: true, key: params.key, status: 'saved' };
          } catch (sdkKvErr) {
            log(`[PUTER-SDK] KV Set native gagal: ${sdkKvErr.message}. Mencoba REST fallback.`);
          }
        }
      ...
      if (action === 'kv_get') {
        if (puter) {
          try {
            log('[PUTER-SDK] Membaca KV native...');
            const val = await puter.kv.get(params.key);
            return { success: true, key: params.key, value: val };
          } catch (sdkKvErr) {
            log(`[PUTER-SDK] KV Get native gagal: ${sdkKvErr.message}. Mencoba REST fallback.`);
          }
        }
      ...
      if (action === 'fs_write') {
        if (puter) {
          try {
            log('[PUTER-SDK] Menulis file native...');
            await puter.fs.write(params.path, params.content);
            return { success: true, path: params.path, status: 'written' };
          } catch (sdkFsErr) {
            log(`[PUTER-SDK] FS Write native gagal: ${sdkFsErr.message}. Mencoba REST fallback.`);
          }
        }
      ...
      if (action === 'fs_read') {
        if (puter) {
          try {
            log('[PUTER-SDK] Membaca file native...');
            const content = await puter.fs.read(params.path);
            return { success: true, path: params.path, content: content };
          } catch (sdkFsErr) {
            log(`[PUTER-SDK] FS Read native gagal: ${sdkFsErr.message}. Mencoba REST fallback.`);
          }
        }
