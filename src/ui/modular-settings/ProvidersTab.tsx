...
export const ProvidersTab: React.FC<ProvidersTabProps> = ({
  ...
  // States to support testing TTS settings within its module setup ("coba di saat seting modul tts bisa di coba dulu")
  const [testSpeechText, setTestSpeechText] = React.useState('Halo Kakak! Senang bisa menemuimu. Aku harap hari-harimu menyenangkan~');
  ...
  const handlePlaytestTts = async (providerId: string) => {
    ...
    try {
      ...
      if (!ttsModule) {
        throw new Error(`Modul TTS '${providerId}' tidak ditemukan di registri.`);
      }
      ...
      setTtsTestError(err.message || 'Gagal menyuarakan uji vokal batin.');
    } finally {
      ...
      {providerSubpage === null ? (
        ...
        <div className="space-y-6 animate-fade-in font-sans">
          ...
          {(() => {
              ...
              if (staticProvider.tab === 'chat') {
                if (staticProvider.id === 'official_chat') {
                  ...
                  fields = {
                    ...
                    model: { type: 'select', label: 'Active Gemini model', default: 'gemini-2.5-flash', options: [{ value: 'gemini-2.5-flash', label: 'gemini-2.5-flash (Default Lightweight)' }, { value: 'gemini-2.0-flash', label: 'gemini-2.0-flash (Ultra-fast speed)' }, { value: 'gemini-1.5-pro', label: 'gemini-1.5-pro (Heavy multi-modal)' }] }
                  };
                } else if (staticProvider.id === 'puter-neural-provider') {
                  fields = {
                    provider: {
                      type: 'select',
                    ...
                    token: { type: 'password', label: 'Puter Auth Token (Optional)', description: 'Enter your Puter Auth Token to authorize features like chat, txt2img, txt2speech and txt2vid. Auto-resolves if left empty.' }
                  };
                }
                ...
                } else if (staticProvider.id === 'puter-tts') {
                  fields = {
                    provider: {
                      type: 'select',
                    ...
                    token: { type: 'password', label: 'Puter Auth Token (Optional)', description: 'Enter specific token if you wish to override global token configured in Puter Cloud AI.' }
                  };
            ...
            const isOptionalKeyProvider = ['puter-neural-provider', 'puter-tts', 'puter', 'local', 'browser_speech', 'browser_hearing', 'ollama', 'lmstudio', 'comfyui', 'none_speech'].includes(providerSubpage || '');
                ...
                {registeredModule ? (
                  <div className="bg-[#0e0e14]/55 border border-white/5 p-6 rounded-2xl space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
                      ...
                      {(() => {
                        const isChat = staticProvider?.tab === 'chat' || ['gemini', 'openai', 'anthropic', 'openrouter', 'local', 'puter', 'custom'].includes(registeredModule.metadata.id);
                    ...
                    {staticProvider?.tab === 'speech' && (
                      <div className="mt-6 border-t border-white/5 pt-5 space-y-3 font-sans">
                        ...
                        <p className="text-[10px] text-zinc-400 leading-relaxed">
                          Kemampuan olah vokal batin Yui ({staticProvider.name}) dapat dicoba langsung sebelum mengaktifkannya secara global.
                        </p>
                        <div className="flex gap-2">
                          <input
                            ...
                            placeholder="Tulis kalimat uji coba batin di sini..."
