...
export function initializeCortexModules(): Promise<void> {
  ...
  if (typeof window !== "undefined") {
    ...
    // Register Explicit Providers to ensure reliable coverage in compiled environments
    try {
      const { PuterProvider } = await import('../drivers/ai-providers/PuterProvider');
      SystemRegistry.register(PuterProvider);
    } catch (e) {
      console.warn('[REGISTRY] Native static import of PuterProvider failed, relying on glob-discover fallback.', e);
    }
