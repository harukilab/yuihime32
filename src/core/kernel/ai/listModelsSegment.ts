...
export async function listModels(
  ...
  try {
    ...
    if (cleanProvider === 'puter' || cleanProvider === 'puter-neural-provider') {
      try {
        const puterService = PuterService.getInstance();
        const puterModels = await puterService.listModels();
        if (Array.isArray(puterModels) && puterModels.length > 0) {
          return {
            models: puterModels.map((m: any) => ({
              name: m.provider ? `${m.provider}:${m.id || m.name}` : (m.id || m.name),
              displayName: m.provider ? `${m.provider.toUpperCase()}: ${m.name || m.id}` : (m.name || m.id),
              supportedGenerationMethods: ['generateContent']
            }))
          };
        }
      } catch (e: any) {
        console.log('[SERVER_AI] Puter dynamic model list failed, using static fallback:', e?.message || e);
      }
      return {
        models: [
          { name: 'openai:gpt-4o-mini', displayName: 'OpenAI: GPT-4o Mini (Default)', supportedGenerationMethods: ['generateContent'] },
          { name: 'openai:gpt-4o', displayName: 'OpenAI: GPT-4o', supportedGenerationMethods: ['generateContent'] },
          { name: 'anthropic:claude-3-5-sonnet', displayName: 'Anthropic: Claude 3.5 Sonnet', supportedGenerationMethods: ['generateContent'] },
          { name: 'google:gemini-1.5-flash', displayName: 'Google: Gemini 1.5 Flash', supportedGenerationMethods: ['generateContent'] }
        ]
      };
    }
